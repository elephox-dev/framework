...
class LoggerCommand implements CommandHandler
{
	...
	public function configure(CommandTemplateBuilder $builder): void
	{
		...
		$builder->addOption('sink', default: ColoredConsoleSink::class, description: 'The sink to use');
	}
	...
	public function handle(CommandInvocation $command): int|null
	{
		...
		if (!class_exists($sinkClass)) {
			$this->logger->error("Sink class '$sinkClass' does not exist");
		...
		if (!$interfaces || !in_array(Sink::class, $interfaces, true)) {
			$this->logger->error("Sink class '$sinkClass' does not implement Sink");
		...
		if (in_array(SinkProxy::class, $interfaces, true)) {
			$args['innerSink'] = new StandardSink();
		}
